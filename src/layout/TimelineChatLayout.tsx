...
import { TimelineMessageList, type TimelineMessageListRef } from '../components/messages/timeline/MessageList';
...
export interface TimelineChatLayoutProps {
  theme?: Theme;
...
export const TimelineChatLayout = forwardRef<TimelineChatLayoutRef, TimelineChatLayoutProps>(
  (
    {
      theme = 'dark',
      showAgentInfo = false,
    ...
    const messageListRef = useRef<TimelineMessageListRef>(null);
    ...
    // 发送消息处理 —— 与 Vue 版对齐: 构建 RunAgentInfo {name, type}
    ...
    const handleLoadMore = useCallback(() => {
      transport.loadMoreMessages(agents.length > 1 ? undefined : currentAgent);
    }, [transport, agents.length, currentAgent]);
...
TimelineChatLayout.displayName = 'TimelineChatLayout';
